...
acme_standalone(){
    ...
    bash ~/.acme.sh/acme.sh --install-cert -d ${domain} --key-file /etc/ssl/ssl/${domain}.key --fullchain-file /etc/ssl/ssl/${domain}.crt
    checktls
}
...
acme_cfapiNTLD(){
    ...
    export CF_Email="$CFemail"
    if [[ -z $ipv4 ]]; then
        bash ~/.acme.sh/acme.sh --issue --dns dns_cf -d "*.${domain}" -d "${domain}" --listen-v6 --insecure
    else
        bash ~/.acme.sh/acme.sh --issue --dns dns_cf -d "*.${domain}" -d "${domain}" --insecure
    fi
    bash ~/.acme.sh/acme.sh --install-cert -d "*.${domain}" --key-file /etc/ssl/ssl/${domain}.key --fullchain-file /etc/ssl/ssl/${domain}.crt
    checktls
}
...
checktls() {
    if [[ -f /etc/ssl/ssl/${domain}.crt && -f /etc/ssl/ssl/${domain}.key ]]; then
        if [[ -s /etc/ssl/ssl/${domain}.crt && -s /etc/ssl/ssl/${domain}.key ]]; then
            if [[ -n $(type -P wg-quick) && -n $(type -P wgcf) ]]; then
                wg-quick up wgcf >/dev/null 2>&1
            fi
            if [[ -n $(type -P wg-quick) && -n $(type -P wgcf) ]]; then
            ...
            green "证书申请成功! 脚本申请到的证书 (${domain}.crt) 和私钥 (${domain}.key) 文件已保存到 /etc/ssl/ssl 文件夹下"
            yellow "证书crt文件路径如下: /etc/ssl/ssl/${domain}.crt"
            yellow "私钥key文件路径如下: /etc/ssl/ssl/${domain}.key"
            back2menu
        else
            if [[ -n $(type -P wg-quick) && -n $(type -P wgcf) ]]; then
                wg-quick up wgcf >/dev/null 2>&1
...
revoke_cert() {
    ...
    if [[ -n $(bash ~/.acme.sh/acme.sh --list | grep $domain) ]]; then
        ...
        rm -f /etc/ssl/ssl/${domain}.crt /etc/ssl/ssl/${domain}.key
        rm -f /etc/ssl/ssl/${domain}.crt /etc/ssl/ssl/${domain}.crt
